...
int main(int argc, char* argv[]){
   ...
   // cmd.add<string>("umi_loc", 0, "specify the location of UMI, can be (index1/index2/read1/read2/per_index/per_read, default is none", false, "");
    cmd.add<size_t>("umi_len", 0, "if the UMI is in read1/read2, its length should be provided, default=16", false, 16);
   ...
   // cmd.add<int>("umi_skip", 0, "if the UMI is in read1/read2, fastp can skip several bases following UMI, default is 0", false, 0);
	...
	if (opt.umi.enabled) {
		string umiPrefix = cmd.get<string>("umi_prefix");
		str2upper(umiPrefix);
		if (umiPrefix.empty())
			error_exit("You've enabled UMI by(--umi), you should specify the UMI prefix fixed sequence by (--umi_prefix)");
		if (umiPrefix == "P5") {
			opt.umi.prefix = P5;
		}
		else if (umiPrefix == "M13R") {
			opt.umi.prefix = M13R;
		}
		else if (umiPrefix == "TN") {
			opt.umi.prefix = TN;
		}
		else if (umiPrefix == "PRC") {
			opt.umi.prefix = PRC;
		}
		else {
	...
	opt.barcode.barcodeListFile = cmd.get<string>("barcode_list");
	if(opt.barcode.enabled){
			string barcodeList = opt.barcode.barcodeListFile;
			//str2lower(barcodeList);
			if(barcodeList.empty())
				error_exit("You've enabled barcode  by (--barcode), you should specify the barcode list file path by (--barcode_list)");
			if(!opt.isPaired())
				error_exit("The barcode should be in the read2 end, but the input data is not paired end.");
			opt.loadBarcodeList();
	}
