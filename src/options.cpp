...
void Options::loadBarcodeList() {
	if (barcode.barcodeListFile.empty()) {
		barcode.hasBarcodeListFile = false;
		return;
	}

	check_file_valid(barcode.barcodeListFile);

	BarcodeListReader barcodereader(barcode.barcodeListFile);
	barcodereader.readAll();
	barcode.barcode_map = barcodereader.barcode_map;
}
...
bool Options::validate() {
    ...
    if(umi.enabled) {
		if (umi.length > 100)
			error_exit("UMI length shouldn't more than 100");
		if (umi.reads_support < 0 || umi.reads_support>100)
			error_exit("UMI reads support should be in the range 0~100.");
    }

	if (barcode.enabled) {
		if(barcode.length < 1 || barcode.length>100)
			error_exit("barcode length should be 1~100");
		if(barcode.skip < 0 || barcode.skip > 100)
			error_exit("The base number to skip between each barcode should be 0~100");
		if (barcode.barcodeListFile.empty())
			error_exit("The barcode list file must be given when baecode maeker enabled");
	}
