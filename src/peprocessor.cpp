...
PairEndProcessor::PairEndProcessor(Options* opt){
	...
	mBarcodeProcessor = new BarcodeProcessor(opt);
...
bool PairEndProcessor::processPairEnd(ReadPairPack* pack, ThreadConfig* config){
    ...
    for(int p=0;p<pack->count;p++){
		...
		if (mOptions->barcode.enabled)
			...
			mBarcodeProcessor->process(or1, or2, config->getFilterResult());

        // umi processing
        if(mOptions->umi.enabled)
            mUmiProcessor->process(or1, or2, config->getFilterResult());
